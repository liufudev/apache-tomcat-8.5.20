...
function callback(page,next) {
	...
	for (var i = 0; i < data.length; i++) {
		var tagquestion='';
		var taganswer='';
		if(data[i].objType=='CONSULT'){
			tagquestion='[问]:';
			if(data[i].replyContent){
				taganswer='<br>[答]:'+data[i].replyContent;
			}
				...
				var lisr ='<li>'
					...
					+ '	<p>'+tagquestion+ data[i].commentText+taganswer+'</p>'
					+ '	<h6 class="yuan">'
					+ '<a href="javascript:void(0)" _comment="'
					+ data[i].commentUid + '" _href="' + data[i].infoId + ' " _objType="'+data[i].objType+'">活动：'
...
function 	toInfoDetail(infoid){
			...
			redirect(getRootPath_web()+"/html/activity/activity-info.html?activityId="+infoid+"&type=detail")
}
...
function getMycomment() {
	...
	$.ajax({
		...
		dataType : "json",
		async : false,
		url : getRootPath_web() + "/activity/activityPersonCommentList",
		beforeSend : function() {
	...
	$('input:checkbox[name=num_check]:checked').each(
			function(obj) {
				var a_comment = $(this).parent().parent().parent().children(
						'.comment-rg').children('.comment-rg').children('.txt').children('h6')
						.children('a');
